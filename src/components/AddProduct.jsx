...
import { Save, ArrowLeft, Loader2 as Loader, Upload, Trash2, Image as ImageIcon, Plus, Star, AlertTriangle, X } from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4">
            <div className="bg-[#111] p-8 rounded-lg shadow-2xl border border-yellow-700 max-w-sm w-full relative">
                ...
                <p className="font-mono text-sm text-[#888] text-center mb-6">
                    You have not provided any valid external image URLs. Do you wish to save this product without a primary image?
                </p>
...
export const AddProduct = ({ setNotification }) => {
    ...
    // Stores the latest payload data temporarily when the modal is opened
    const [pendingPayload, setPendingPayload] = useState(null);

    // --- 1. IMAGE HANDLING HELPERS ---
    ...
    // --- 2. UI HANDLERS (omitted for brevity) ---
    const handleDrag = (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const executeSave = async (payload) => {
        ...
        try {
            ...
            if (response.ok && data.success) {
                setNotification({ message: `Product created successfully! ID: ${data.id}`, type: 'success' });
                navigate('/admin');
            } else {
                setNotification({ message: `Failed to create product: ${data.error || 'Server error'}`, type: 'error' });
            }
        } catch (err) {
            console.error(err);
    ...
    // --- 4. FORM SUBMISSION (Initial Check) ---
    const handleSubmit = async (e) => {
        e.preventDefault();

        if (parseFloat(formData.price) <= 0) {
            setNotification({ message: "Price must be greater than zero.", type: 'warning' });
            return;
        }
                ...
                <div className="bg-[#111] border border-[#1a1a1a] rounded-lg shadow-2xl overflow-hidden">
                    <div className="p-6 border-b border-[#333] bg-[#0f0f0f] flex justify-between items-center">
                        <h1 className="font-display text-3xl text-white uppercase flex items-center gap-2">
                            <Plus size={28} className="text-[#CCFF00]" /> Add New Product
                        </h1>
                    </div>
